...
USER_API_URL = 'http://user-service-c:5001/user/api'
...
def get_order():
    ...
    open_order = Order.query.filter_by(user_id=user['id'], is_open=1)
    if open_order:
        return jsonify({'result': open_order.serialize()}), 201
    else:
